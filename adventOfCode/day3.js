...
const driveTaxi = road => {
  const taxiPosition = new Set();

  let curPos = { streets: 0, aves:0 };
  ...
  for (let i = 0; i < road.length; i++) {
    ...
    for (let j = 0; j < dist; j++) {
      ...
      taxiPosition.add(JSON.stringify(curPos))
    }
...
const calcManhattanDistance = cityBlock => Math.abs(cityBlock.streets) + Math.abs(cityBlock.aves);

const findNearestDuaneReade = (routeOne, routeTwo) => {
  const firstTaxi = driveTaxi(routeOne);
  const secondTaxi = driveTaxi(routeTwo);
  let taxiFare = Infinity;
  firstTaxi.forEach(cityBlock => {
    if (secondTaxi.has(cityBlock)) 
      taxiFare = Math.min(taxiFare, calcManhattanDistance(JSON.parse(cityBlock)));
  });
  return taxiFare;
};


test(findNearestDuaneReade(inputOne, inputTwo), 225, 'Part One')
