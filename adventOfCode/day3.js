...
const layWire = wire => {
  ...
  for (let i = 0; i < wire.length; i++) {
    const direction = dir[wire[i][0]];
    const dist = Number(wire[i].slice(1));
    for (let j = 1; j < dist; j++) {
      curPos.y += direction.y;
      curPos.x += direction.x;
      wirePositions.add(JSON.stringify(curPos))
    }
...
const findCrossedWires = (wireOne, wireTwo) => {

};

const test1 = layWire(wireOne);
test(test1.size, '???', 'Part One')
